...
local Library do 
    ...
    local Instances = { } do
        ...
        Instances.Tooltip = function(self, Text)
            ...
            Library:Connect(Gui.MouseEnter, Show)
            Library:Connect(Gui.MouseLeave, Hide)
    ...
    Library.IsMouseOverFrame = function(self, Frame)
        ...
        if Pointer.X >= AbsolutePosition.X and Pointer.X <= AbsolutePosition.X + AbsoluteSize.X and Pointer.Y >= AbsolutePosition.Y and Pointer.Y <= AbsolutePosition.Y + AbsoluteSize.Y then
            return true
        end
    end
        ...
        Components.Page = function(Data)
            ...
            local Items = { } do 
                Items["Inactive"] = Instances:Create("TextButton", {
                    ...
                    TextColor3 = FromRGB(180, 180, 180),
                    BorderColor3 = FromRGB(0, 0, 0),
                    Text = type(Data.Name) == "string" and Data.Name or tostring(Data.Name),
                    AutoButtonColor = false,
                    BackgroundTransparency = 1,
                    Name = "\0",
                    AutomaticSize = Enum.AutomaticSize.X,
    ...
    Components.Button = function(Data)
        ...
        function Button:SubButton(Data)
            ...
            SubItems["Button"]:Connect("Activated", function()
                SubButton:Press()
            end)
        end
    ...
    Components.Slider = function(Data)
        ...
        function Slider:Set(Value)
            ...
            if Value == nil or Value ~= Value then -- Check for NaN
                Value = Data.Min
            end
            ...
            if range > 0 then
                local alpha = (Slider.Value - Data.Min) / range
                if alpha ~= alpha then alpha = 0 end -- Check for NaN
                Items["Indicator"]:Tween(TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Size = UDim2New(alpha, 0, 1, 0)})
            end
            
            local displayValue = Slider.Value
            if displayValue ~= displayValue then displayValue = 0 end -- Check for NaN
            Items["Value"].Instance.Text = StringFormat("%s%s", tostring(displayValue), type(Data.Suffix) == "string" and Data.Suffix or "")
        ...
        local function UpdateFromInput(Input)
            ...
            if AbsSize > 0 then
                local Alpha = MathClamp((X - AbsPos) / AbsSize, 0, 1)
                local Value = Data.Min + (Data.Max - Data.Min) * Alpha
                Slider:Set(Value)
            end
        end
        ...
        Items["RealSlider"]:Connect("InputEnded", function(Input)
            if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                if not DragInput or Input == DragInput then
                    Dragging = false
                    DragInput = nil
                end
            end
        ...
        Library:Connect(UserInputService.InputChanged, function(Input)
            ...
            if DragInput and Input.UserInputType == Enum.UserInputType.Touch and Input ~= DragInput then
                return
            end
    ...
    Components.Dropdown = function(Data)
        ...
        function Dropdown:OpenPopup()
            ...
            for _, Child in ipairs(Library.Holder:GetDescendants()) do
                if Child.Name == "DropdownBlocker" or Child.Name == "DropdownPopup" then
                    Child:Destroy()
                end
            end
            ...
            -- Create blocker to capture outside clicks
            PopupBlocker = Instances:Create("TextButton", {
            ...
            PopupBlocker.Instance.MouseButton1Click:Connect(function()
                Dropdown:ClosePopup()
            end)

            -- Get screen position
            local AbsPos = Items["RealDropdown"].Instance.AbsolutePosition
            local AbsSize = Items["RealDropdown"].Instance.AbsoluteSize

            -- Create popup
            local MaxHeight = Data.MaxSize or 150
            PopupFrame = Instances:Create("ScrollingFrame", {
                Name = "DropdownPopup",
                Parent = Library.Holder,
                BackgroundColor3 = FromRGB(19, 19, 19),
                BorderSizePixel = 0,
                Position = UDim2New(0, AbsPos.X, 0, AbsPos.Y + AbsSize.Y + 3),
                Size = UDim2New(0, AbsSize.X, 0, 0),
                AutomaticCanvasSize = Enum.AutomaticSize.Y,
                CanvasSize = UDim2New(0, 0, 0, 0),
                ScrollBarThickness = 2,
                ScrollBarImageColor3 = FromRGB(150, 100, 255),
                ...
                TopImage = "rbxassetid://85239668542938",
                BottomImage = "rbxassetid://85239668542938"
            })  PopupFrame:AddToTheme({BackgroundColor3 = "Section Background", ScrollBarImageColor3 = "Accent"})
            ...
            local PopupList = Instances:Create("UIListLayout", {
                Parent = PopupFrame.Instance,
                Padding = UDimNew(0, 2),
                SortOrder = Enum.SortOrder.LayoutOrder
            })
            ...
            for Index, ItemName in ipairs(Dropdown.ItemsList) do
                local OptionBtn = Instances:Create("TextButton", {
                    ...
                    BackgroundColor3 = FromRGB(255, 255, 255),
                    ZIndex = 202
                })  OptionBtn:AddToTheme({TextColor3 = "Text"})
                ...
                -- Highlight selected
                if Data.Multi then
                    if TableFind(Dropdown.Value, ItemName) then
                        OptionBtn:ChangeItemTheme({TextColor3 = "Accent"})
                        OptionBtn.Instance.TextColor3 = Library.Theme.Accent
                    end
                else
                ...
                OptionBtn.Instance.MouseButton1Click:Connect(function()
                    if Data.Multi then
                        local Index = TableFind(Dropdown.Value, ItemName)
                        if Index then
                            TableRemove(Dropdown.Value, Index)
                        else
                            TableInsert(Dropdown.Value, ItemName)
                        end
                        ...
                        Library.Flags[Dropdown.Flag] = Dropdown.Value
                        Items["Value"].Instance.Text = tostring(ItemName)
                        Dropdown:ClosePopup()
                    end
                ...
                OptionBtn.Instance.MouseEnter:Connect(function()
                    OptionBtn:Tween(nil, {TextColor3 = Library.Theme.Accent})
                end)

                OptionBtn.Instance.MouseLeave:Connect(function()
                    if Data.Multi then
                        if TableFind(Dropdown.Value, ItemName) then
                            OptionBtn:Tween(nil, {TextColor3 = Library.Theme.Accent})
                        else
                            OptionBtn:Tween(nil, {TextColor3 = Library.Theme.Text})
                        end
                    else
                        if Dropdown.Value == ItemName then
                            OptionBtn:Tween(nil, {TextColor3 = Library.Theme.Accent})
            ...
            -- Set popup size with max height
            task.wait()
            local ContentHeight = PopupList.Instance.AbsoluteContentSize.Y + 12
            local FinalHeight = MathMin(ContentHeight, MaxHeight)
            PopupFrame.Instance.Size = UDim2New(0, AbsSize.X, 0, FinalHeight)
        end
        ...
        Items["RealDropdown"].Instance.MouseButton1Click:Connect(function()
    ...
    Components.Label = function(Data)
        local Items = { } do 
            local AlignmentKey = Data.Alignment or "Left"
            if typeof(AlignmentKey) == "EnumItem" then
                AlignmentKey = AlignmentKey.Name
            end
            if type(AlignmentKey) ~= "string" then
                AlignmentKey = "Left"
            end
            ...
            Items["Text"] = Instances:Create("TextLabel", {
                ...
                TextColor3 = FromRGB(180, 180, 180),
                BorderColor3 = FromRGB(0, 0, 0),
                Text = type(Data.Text) == "string" and Data.Text or tostring(Data.Text),
                Name = "\0",
                Size = UDim2New(1, 0, 1, 0),
                BackgroundTransparency = 1,
                TextXAlignment = Enum.TextXAlignment[AlignmentKey] or Enum.TextXAlignment.Left,
                BorderSizePixel = 0,
                RichText = true,
                TextSize = 12,
                BackgroundColor3 = FromRGB(255, 255, 255)
    ...
    Components.Keybind = function(Data)
        ...
        Keybind.TouchOpen = false
        Items["KeyButton"]:Connect("InputBegan", function(Input)
            if Input.UserInputType == Enum.UserInputType.Touch then
                Keybind.TouchOpen = true
            else
                Keybind.TouchOpen = false
            end
        end)

        Items["KeyButton"]:Connect("Activated", function()
            if Keybind.TouchOpen then
                Keybind.TouchOpen = false
        ...
        Items["Always"]:Connect("Activated", function()
            Keybind.Mode = "Always"
            Keybind:SetMode("Always")
        end)

        Items["Hold"]:Connect("Activated", function()
            Keybind.Mode = "Hold"
    ...
    Components.Listbox = function(Data)
        ...
        function Listbox:Add(Option)
            ...
            OptionData.OptionButton:Connect("Activated", function()
                OptionData:Set()
            end)
    ...
    Library.Window = function(self, Data)
        ...
        local Window = {
            ...
            Size = Data.Size or Data.size or UDim2New(0, 500, 0, 558),

			GradientTitle = Data.GradientTitle or Data.gradienttitle or { Enabled = false },
        ...
        local Items = Components.Window({
            ...
            Items["Pages"] = Instances:Create("Frame", {
                Parent = Items["Inline"].Instance,
                Name = "\0",
                Position = UDim2New(0, 13, 0, 35),
                BorderColor3 = FromRGB(0, 0, 0),
                Size = UDim2New(1, -28, 0, 37),
                BorderSizePixel = 2,
                BackgroundColor3 = FromRGB(13, 13, 13)
            })  Items["Pages"]:AddToTheme({BackgroundColor3 = "Inline", BorderColor3 = "Outline"})
        ...
        -- Auto-scale for smaller screens (keeps PC size unless it would go off-screen)
        local WindowScale = Instances:Create("UIScale", {
            Parent = Items["Outline"].Instance,
            Scale = 1
        })
        ...
        local function ApplyAutoScale()
            ...
            local desiredX = Window.Size.X.Offset
            local desiredY = Window.Size.Y.Offset
            if desiredX <= 0 or desiredY <= 0 then
                WindowScale.Instance.Scale = 1
                return
            end

            local pad = 24
            local sx = (viewport.X - pad) / desiredX
            local sy = (viewport.Y - pad) / desiredY
            local scale = MathClamp(math.min(1, sx, sy), 0.6, 1)
            WindowScale.Instance.Scale = scale
        end

        ApplyAutoScale()
        pcall(function()
            Library:Connect(Camera:GetPropertyChangedSignal("ViewportSize"), ApplyAutoScale)
        end)

        -- External open/close button (top-right, draggable) - bigger black box
        ...
        ToggleButton:Connect("Activated", function()
        ...
        if UserInputService.TouchEnabled then
            Library.MobileButtonCount = (Library.MobileButtonCount or 0) + 1
            local Index = Library.MobileButtonCount
            ...
            local MenuButton = Instances:Create("TextButton", {
                ...
                Position = UDim2New(0, 12, 0, 12 + ((Index - 1) * 42)),
            ...
            MenuButton:AddToTheme({BackgroundColor3 = "Element", TextColor3 = "Text"})
            MenuButton:Connect("Activated", function()
                Window:SetOpen(not Window.IsOpen)
            end)
    ...
    Library.Pages.PlayerList = function(self, Data)
        ...
        function Playerlist:Add(Player)
            ...
            PlayerData.PlayerButton:Connect("Activated", function()
                PlayerData:Set()
            end)
    ...
    Library.Sections.Label = function(self, Text, Alignment)
        if type(Text) == "table" then
            Alignment = Text.Alignment or Text.alignment or Alignment
            Text = Text.Name or Text.name or Text.Text or Text.text
        end
        ...
        local Label = {
            ...
            Name = (type(Text) == "string" and Text) or tostring(Text or "Label"),
